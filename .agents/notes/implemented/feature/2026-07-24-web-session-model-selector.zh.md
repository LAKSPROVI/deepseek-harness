...
会话 RPC 领域公开 `session.models` 模型目录与 `session.selectModel`。该目录从 LLM（大语言模型）注册表动态构建，并按提供方分组；每个已列出模型的精确元数据还会加入由适配器持有的推理强度 ID、名称、说明、可选默认值与可选输入模态。各提供方的目录与精确元数据会按提供方并发加载，且彼此独立失败，因此成功加载的分组仍可与可重试的失败记录一同使用。模型是否位于目录仅供参考：`session.models.current` 与独立解析的 `currentModel` 元数据即使不在任何分组中也仍然可以路由，且提供方停止公布该模型后，Host 不会合成未列出行。`inputModalities` 缺席表示未知并保持允许；显式列表缺少 `image` 才表示已知不支持图片。两个前端对未列出的当前选择给出不同回答：TUI 将其渲染为独立一行，Web 则显示未设置状态的触发器标签并要求选择替代模型。Web 仍可使用该可路由选择发送消息。精确解析决定提供方／模型组合与显式推理强度是否可用。选择操作通过 `resolveCallConfig` 拒绝不支持的推理强度 ID并具体化适配器配置的默认值，然后回显已解析的 `currentModel`，使浏览器能力状态与被接受的选择原子更新。

浏览器中的 `ModelDirectoryResolver` 为每个实时会话持有一个 `ModelDirectory`。其快照包含当前完整的 `ModelSelection`、精确 `currentModel` 元数据、分组目录、提供方失败记录、操作错误，以及 `idle`、`loading`、`ready`、`selecting`、`error` 状态。挂载时会预先填充触发器标签，此后每次打开菜单都会刷新目录。目录与选择调用共用操作代次，防止较早响应覆盖较新结果；连接重置会先丢弃当前进程中的投影，再恢复 Host 选择。失败时保留先前的选择和可用分组。resolver 还会把目录 store 与输入 store 合成为 composer block：只有精确当前模态已知且不含 `image` 时，草稿图片才会被阻断。移除最后一张图片或选择支持图片的模型会立即清除 block，未知元数据则保持允许。匹配的同步准入检查会在附件编码、上传或 prompt RPC 之前拒绝，并保持输入事务不变。Host 的图片检查始终保留权威。
...
Host 与 RPC schema 测试固定分组发现、输入模态三态语义、建议目录之外的精确当前元数据、选择能力回显、目录与精确元数据失败隔离、已记录推理强度恢复且不注入陈旧行、不支持的推理强度拒绝、默认值具体化，以及切换仅影响下一次组装。客户端测试固定 load/select 对 `currentModel` 的替换、已知不兼容图片阻断、未知与支持图片状态保持允许、编码或 RPC 前保留草稿、移除图片／切换模型后的恢复，以及会话 scope dispose。组件测试固定动态推理强度标签、说明、提供方默认值展示、推理强度提交，以及缺席模型行的 `Select model` 回退。无密钥 built-app 图片快照通过生产模型与 conversation 插件，在已知仅文本的当前模型上附加图片，固定文本与图片保留时的禁用提示，并通过移除图片恢复 composer。
