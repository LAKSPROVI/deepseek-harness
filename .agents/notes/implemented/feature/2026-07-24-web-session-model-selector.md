...
The session RPC domain exposes a `session.models` directory and `session.selectModel`. The directory is built dynamically from the LLM registry and grouped by provider; each listed model's exact metadata adds adapter-owned reasoning effort ids, names, descriptions, optional default, and optional input modalities. Provider catalogs and exact metadata load concurrently by provider and fail independently, so successful groups remain usable alongside retryable failure records. Catalog membership stays advisory: `session.models.current` and independently resolved `currentModel` metadata can remain routable when absent from every group, and the Host does not synthesize an unlisted row after its provider stops advertising it. Missing `inputModalities` means unknown and remains permissive; an explicit list omitting `image` is a known negative. The two surfaces answer an unlisted current selection differently: the TUI renders it as its own row, while Web shows the unset trigger label and asks for a replacement. Web can still send with that routable selection. Exact resolution decides whether a provider/model pair and explicit effort are available. Selection uses `resolveCallConfig` to reject unsupported effort ids and materialize an adapter-configured default, then echoes the resolved `currentModel` so browser capability state changes atomically with the accepted selection.

The browser `ModelDirectoryResolver` owns one `ModelDirectory` per live session. Its snapshot contains the current complete `ModelSelection`, exact `currentModel` metadata, grouped catalog, provider failures, operation error, and `idle`/`loading`/`ready`/`selecting`/`error` state. Mounting primes the trigger label and each menu open refreshes the directory. Directory and selection calls share an operation generation so an older response cannot replace a newer result; connection reset discards the process-local projection before restoring the Host selection. Failures retain the previous selection and usable groups. The resolver also combines the directory and input stores into a composer block: a draft image is blocked only when the exact current modalities are known and omit `image`. Removing the last image or selecting an image-capable model clears the block immediately, while unknown metadata remains permissive. A matching synchronous admission check rejects before attachment encoding, upload, or prompt RPC and leaves the input transaction intact. The Host image check remains authoritative.
...
Host and RPC schema tests pin grouped discovery, tri-state input modalities, exact current metadata outside the advisory catalog, selection capability echo, catalog and exact-metadata failure isolation, logged effort restoration without stale-row injection, unsupported effort rejection, default materialization, and next-assembly switching. Client tests pin load/select replacement of `currentModel`, known-negative image blocking, permissive unknown and image-capable states, draft preservation before encoding or RPC, removal/model-switch recovery, and session-scope disposal. Component tests pin dynamic effort labels, descriptions, provider-default exposure, effort submission, and the `Select model` fallback for an absent row. The keyless built-app image snapshot uses the production model and conversation plugins to attach an image to a known text-only current model, pins the disabled guidance while text and image remain, and removes the image to restore the composer.
