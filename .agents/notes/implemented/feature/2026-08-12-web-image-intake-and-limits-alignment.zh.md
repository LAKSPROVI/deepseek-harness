...
**工具栏选择器。** Command 启动器旁的图片按钮打开隐藏的原生多文件 input，仅接受 PNG、JPEG、WebP 与 GIF。每次选择后都会清空 input 的值，因此再次选择同一文件仍会触发 `change`；随后与粘贴和整页拖放一样进入同一个 `intakeImages` 路径。Command 启动器保持独立；选择器不引入通用文件协议或第二套菜单组件。
