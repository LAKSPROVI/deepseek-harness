...
The Lead Session stores one current structured debate as whole `team/debate` snapshots. A debate carries a stable id, monotonic revision, topic, participant names, round, maximum rounds, status, current phase, and compact transition history. The ordered phases are `positions`, `critique`, `rebuttal`, `verification`, and `synthesis`; advancing from final synthesis either begins the next round or completes the debate at its round cap.

Create and transition operations are Lead-authorized. Every transition supplies the expected revision and fails on stale state. `pause` and `resume` change durable debate status without cancelling an Agent turn or modifying its inbox; `interrupt` remains the separate subagent operation for current-turn cancellation. Human controls invoke the same durable operations through the generated Remote, while model tools remain scoped by the `agent-teams` preset.
