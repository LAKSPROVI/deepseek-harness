...
The browser projects a **Recent & In Progress** highlight section pinned frozen above the Workspace groups tree, aggregating active (running, subagent activity, pending interaction) and unread sessions with compact Workspace badge tags and 1-click navigation while the Workspace tree below scrolls independently. A session remains in the recent section across clicks until the user explicitly assigns a finalizing status indicator (such as marking it completed or clearing status). Session row menus provide a **Set status** submenu (`Ongoing`, `Waiting decision`, `Unread`, `Completed`, `Clear status`) as well as direct toggles for **Mark as unread / Mark as read** and **Mark as completed / Mark as incomplete**; marking completed removes the session from the top section while displaying a solid green indicator on its group row below.
