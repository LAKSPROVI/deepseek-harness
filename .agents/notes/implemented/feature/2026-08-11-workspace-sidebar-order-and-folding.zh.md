...
浏览器在 Workspace 分组树上方呈现固定置顶的**进行中与近期会话**（Recent & In Progress）高亮区域，汇聚活跃会话（运行中、子智能体运行、等待交互）与未读会话，附带紧凑的所属 Workspace 徽标与一键直达导航，而下方 Workspace 目录树独立滚动。会话在被点击打开后仍持续保留在近期区域内，直至用户显式对其设置结束指示器（如标为已完成或清除状态）。Session 行菜单提供**设置状态**子菜单（`进行中`、`等待决策`、`未读`、`已完成`、`清除状态`）以及**标为未读／标为已读**与**标为已完成／标为未完成**快捷切换操作；标为已完成后移出顶部近期区域，并在下方分组行中显示绿色已完成状态点。
