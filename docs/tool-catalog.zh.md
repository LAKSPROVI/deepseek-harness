...
| `@deepseek-ai/dsh-tool-automation` | `automation_create_task`、`automation_delete_task`、`automation_list_tasks`、`automation_pause_task`、`automation_resume_task`、`automation_trigger_task` | `ctx.tools`、`ctx.automation` | `tool/call`、`tool/result after the engine store or controller answers` | - | automation_create_task 默认使用 CUSTOM_PROMPT 动作；运行本身即发即忘，只有通过之后的列出才会到达模型。 |
...
调度一个在重启后仍然存在的持久化任务。默认情况下，任务到期时会在工作区打开一个新会话并发送 `prompt`（action_type CUSTOM_PROMPT）；工作区默认为本会话的目录。schedule_expr 取决于 schedule_type：ONCE 为 ISO-8601 时刻，INTERVAL 为秒数，CRON 为五段 UTC cron 表达式，RRULE 为 RRULE 字符串。
...
立即运行持久化任务，不受其调度限制。返回 run id；运行本身在后台进行。
...
automation_create_task 默认使用 CUSTOM_PROMPT 动作；运行本身即发即忘，只有通过之后的列出才会到达模型。
