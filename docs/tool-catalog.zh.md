...
| `@deepseek-ai/dsh-tool-agent-team` | `followup_task`、`interrupt_agent`、`list_agents`、`send_message`、`spawn_teammate`、`team_debate_get`、`team_debate_start`、`team_debate_update`、`team_task_create`、`team_task_get`、`team_task_list`、`team_task_update`、`wait_agent` | `ctx.tools`、`ctx.systemPrompt`、`ctx.agentTeams`、`an exact live Team member Agent` | `tool/call`、`team/member`、`team/message/queued`、`team/message/delivered`、`team/task`、`tool/result` | - | 所有 Team 工具都限定于隐式 Team Lead 与持久 teammate 作用域。随产品发布的 dsh-base bundle 默认禁用该包；`agent-teams` preset 会启用它，并禁用旧 continuable child 的同名控制工具。 |
...
读取当前结构化 debate，包括其 CAS revision、round、phase、status 与 transition history。
...
启动一个结构化 Team debate。仅 Team Lead 可用。
...
对结构化 debate transition 执行 compare-and-set。仅 Team Lead 可用。
...
所有 Team 工具都限定于隐式 Team Lead 与持久 teammate 作用域。随产品发布的 dsh-base bundle 默认禁用该包；`agent-teams` preset 会启用它，并禁用旧 continuable child 的同名控制工具。
