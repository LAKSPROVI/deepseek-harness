...
| [automation.md](automation.md) | the task automation seam: durable scheduled tasks, recurrence, worker, reaper, SSE panel updates, `ctx.automation` Remote and Chat surfaces |
| [transcription.md](transcription.md) | the transcription seam: provider selection, `TranscriptionRequest`/`Result`, `ctx.transcription` policy, `ctx.voiceInput` Remote namespace |
