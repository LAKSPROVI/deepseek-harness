...
| [agent-team.md](agent-team.md) | Agent Teams: stable Host domain, implicit Lead identity, named continuable teammates, durable mailbox/task/debate state, and the **Equipe de agentes** Web control map |
...
| [transcription.md](transcription.md) | the transcription seam: `TranscriptionRequest`/`Result`, the `TranscriptionProvider` contract, the `maxAudioBytes` ceiling, and the browser-facing `voiceInput` Remote failure union |
