...
| [automation.md](automation.zh.md) | 任务自动化接缝：持久化计划任务、周期、worker、reaper、SSE 面板更新、`ctx.automation` 的 Remote 与 Chat 界面 |
| [transcription.md](transcription.zh.md) | 转写接缝：provider 选择、`TranscriptionRequest`/`Result`、`ctx.transcription` 策略、`ctx.voiceInput` Remote 命名空间 |
