...
| [agent-team.md](agent-team.zh.md) | Agent Teams：稳定 Host 领域、隐式 Lead 身份、具名 continuable teammate、持久 mailbox／task／debate 状态，以及 **Equipe de agentes** Web control 地图 |
...
| [transcription.md](transcription.zh.md) | 转写 seam：`TranscriptionRequest`/`Result`、`TranscriptionProvider` 约定、`maxAudioBytes` 上限，以及面向浏览器的 `voiceInput` Remote 失败联合类型 |
