...
[Agent Teams](subsystems/agent-team.zh.md) 是 `ctx.agentTeams` 上的稳定协作领域，在可继续 subagent 之上提供持久 roster、任务板、mailbox 与结构化 debate。Host service 属于 base composition；界面显示为 **Equipe de agentes** 的显式启用 `agent-teams` preset 会增加模型工具与协作策略。
