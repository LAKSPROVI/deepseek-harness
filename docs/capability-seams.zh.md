...
flowchart LR
  ...
  svc_transcription["ctx.transcription<br/>Speech-to-text seam"]
  pkg_transcription_groq["transcription-groq"]
  pkg_voice_input["voice-input"]
  svc_voiceInput["ctx.voiceInput<br/>Browser-facing voice transcription"]
...
| `ctx.transcription` | `seam` | [`transcription`](../packages/transcription/transcription) | [`transcription-groq`](../packages/transcription/transcription-groq) | [`voice-input`](../packages/transcription/voice-input) | - | 拥有提供方注册、执行期选择、音频字节上限和转写文本裁剪；音频保持临时状态，绝不成为持久或模型可见数据。 |
| `ctx.voiceInput` | `core` | [`voice-input`](../packages/transcription/voice-input) | - | - | - | 解码一次 base64 上传，把全部转写策略委托给该 seam，并通过 Host 一元 Remote 契约回答封闭的业务失败联合类型。 |
...
| `ctx.agentTeams` | `core` | [`agent-team`](../packages/subagent/agent-team) | - | [`tool-agent-team`](../packages/subagent/tool-agent-team) | - | 负责隐式 Root roster、持久 peer mailbox、共享任务 DAG 与 continuable child 生命周期；tool-agent-team 提供作用域化模型策略和控制工具。 |
