...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-voice-input': { kind: 'none', reason: 'Browser-side push-to-talk control; audio stays transient and only a transcript sent by the human reaches a model through the ordinary user-message path.' },
  ...
  'packages/client/ui-prompt-library': { kind: 'none', reason: 'Browser-side saved-prompt controls; only text the human separately submits enters the Session log and model context.' },
  ...
  'packages/automation/automation': { kind: 'none', reason: 'The engine registers no tool, prompt, or session event; a deployment-registered action handler owns any model-facing run.' },
  'packages/experimental/client-ui-automation': { kind: 'none', reason: 'Browser-side header panel over the automations Remote namespace; registers nothing model-facing.' },
