...
链键为「`(normalized tool name, canonical arguments)`」。工具名会被归一化为小写的字母数字片段，并折叠连续重复的片段，因此 `run_code_ide`、`run_code_ide_ide` 与 `runCodeIde` 得到同一个键——模型把某个名字写错时，往往是给它加装饰而不是逐字重复；若直接用原始名字作键，每次变异都会被读成一次全新调用，反而重置了本来要用来抓循环的计数器。参数的规范化则是对键进行深度排序后执行 `JSON.stringify`，因此仅属性顺序不同的参数对象会视为相同。归一化只作用于键：`include`／`exclude` 仍按注册名匹配，详细提醒里引用的也仍是模型实际写下的名字。若某次调用与上一条受跟踪调用相同，该 agent 的连续计数器递增；换成另一条受跟踪调用则重置为 1。
...
- **参数仅检测精确匹配**：工具名会做归一化（大小写、分隔符、重复片段），但参数的规范化只是对键做深度排序，因此近似变体（稍作修改的路径、值内增加的空白）仍可以绕过链；在没有需求证据前，不对参数采用模糊匹配。
