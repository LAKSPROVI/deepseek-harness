...
`session.prompt` 接受一个有序内容列表，其中可以混合文本、base64 编码的图片与 base64 编码的通用文件。宿主把每种附件模态各自作为一个有序批次交给附件存储准入，再按提交时的顺序重建持久消息，因此夹在两段文本之间的文件仍留在原位。请求中不存在的模态完全不会触发准入，因此只含图片的 prompt 在不服务通用文件的存储上依然可调用。整套策略——数量、单项字节、聚合字节、媒体类型、规范 base64——由存储持有，本网关不再附加自己的规则：被拒绝时返回 `attachment-error` 并带上存储给出的确切原因码，且批次中任一成员失败时不提交、不记日志。只有图片会额外按所选路由公布的输入模态校验（`MODEL_DOES_NOT_SUPPORT_IMAGES`）；持久的文件加入绝不依赖模型支持，因为这些字节是被存储、而非被发送。附件准入与模型选择按 Agent 串行，因此并发切换不可能落在模态校验与提交之间。`session.updateQueue` 的编辑仍只接受文本（`QUEUE_EDIT_NON_TEXT`）。

`session.attachment` 把一个持久附件读回为带判别标签的 `image` 或 `file` 值：存储的引用加上 base64 字节，并由存储在读取时校验。授权依据就是会话日志本身——网关扫描该会话的事件，包括嵌套的工具结果与已完成的流式块，只有当日志中的引用确实指向该 id 时才提供服务；未被引用的 id 返回 `attachment-error` 及 `ATTACHMENT_NOT_REFERENCED`，而不是 not-found，并由判别标签决定走哪条读取路径，因此文件 id 绝不会进入图片读取器。文件是不透明的：响应原样返回存储的媒体类型与确切字节长度，不解释其字节；Web 客户端刻意把下载用的 blob 构造为 `application/octet-stream`，因此下载到的文件是被保存，而不是在页面内渲染。
