...
会话每次访问时都按三级解析模型选择：本进程内作出的选择，其次是该会话日志中最新的 `request/header`，最后是这个默认值。已经跑过一轮的会话从自己的日志推导选择，空白会话则能观察到创建之后保存的默认值。在返回模型目录或接纳 prompt 之前，精确模型元数据会把已不受支持的所选推理强度替换为当前模型默认值，或省略该字段以采用提供方默认行为；无法解析的模型保持不变，使路由恢复仍能明确指出不可用的选择。
...
会话模型选择属于会话领域约定。`session.models` 将当前 `ModelSelection` 及其独立解析的 `currentModel` 元数据，与按提供方分组的建议性模型和逐提供方查询失败记录分开返回。该选择可能不在这些分组中，也绝不会作为合成行注入；只要精确解析成功，当前模型元数据仍携带推理信息与 `inputModalities`。`inputModalities` 缺席表示未知并保持允许；显式列表缺少 `image` 才表示已知不支持图片。`session.selectModel` 校验由适配器持有的可选推理强度，指定下次组装提示词时使用的完整选择，并回显已解析的 `currentModel`，使客户端立即替换陈旧能力。目录成员关系不构成校验：适配器可以解析未列出的模型，而不可用的提供方或不受支持的推理强度会返回 `model-unavailable`。`session.models` 还会报告 `routable`，即当前是否有适配器为所选提供方提供服务。该值刻意不从分组推导，因为适配器可以服务未公布的模型。`session.prompt` 始终是权威检查：未服务的路由在开启轮次前以 `model-unavailable` 拒绝，向已知不兼容路由发送图片则以 `MODEL_DOES_NOT_SUPPORT_IMAGES` 拒绝；客户端阻断仅是提示性设计。
...
`session.prompt` 接受一个有序内容列表，其中可以混合文本、base64 编码的图片，以及由 legacy base64 或原始上传 route 返回的会话 scope `UploadedFileAttachment` receipt 表示的通用文件。Host 对每种附件模态执行准入，为确切会话验证 receipt，校验完整文件引用批次，再按提交顺序重建持久消息，因此夹在两段文本之间的文件仍留在原位。请求中不存在的模态完全不会触发准入，因此只含图片的 prompt 在不服务通用文件的存储上依然可调用。整套策略——数量、单项字节、聚合字节、媒体类型、规范 base64——由存储持有，本网关不再附加自己的规则：被拒绝时返回 `attachment-error` 并带上存储给出的确切原因码，且批次中任一成员失败时不提交、不记日志。只有图片会额外按所选路由公布的输入模态校验（`MODEL_DOES_NOT_SUPPORT_IMAGES`）；持久的文件加入绝不依赖模型支持，因为这些字节是被存储、而非被发送。附件准入与模型选择按 Agent 串行，因此并发切换不可能落在模态校验与提交之间。`session.updateQueue` 的编辑仍只接受文本（`QUEUE_EDIT_NON_TEXT`）。

`session.attachment` 为图片与兼容客户端保留带判别标签的 base64 读取。served Web 使用 Host-only `POST` 与 `GET /api/session.file`：POST 在流式写入存储前校验会话，返回小型会话 scope receipt，且不追加事件；之后只有 prompt 或命令接纳才记录经过验证的引用。GET 只接受 `sessionId` 与 `attachmentId`，从该会话权威的 `user/message.content` 解析元数据，并以 `application/octet-stream`、attachment disposition、`nosniff` 和 `private, no-store` 流式传输经过校验的字节；工具元数据与任意日志 JSON 不授权下载。未引用的 id 会在读取存储前返回 404。
