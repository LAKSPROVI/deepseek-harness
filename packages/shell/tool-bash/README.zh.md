...
当已挂载的执行器约束命令（例如 `dsh-bash-sandbox`）时，被阻止的文件操作会报告为 `[sandbox: file access denied under <mode> mode]`——这是策略拒绝，不是命令失败。模型随后可以在同一轮次中用 `sandbox_permissions`（满足需要的最窄更宽模式）与一句 `justification` 重试完全相同的命令一次；该重试引发的审批提示就是用户同意的方式。升权绝不能预先推测：更窄或无效的目标会在不运行任何东西的情况下失败关闭，被拒绝的升权对该命令即为最终结果。与当前模式相同的合法 schema 目标作为幂等无操作运行，不提示审批也不改变权限。
