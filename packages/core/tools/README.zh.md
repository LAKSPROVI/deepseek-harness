...
- `ctx.tools.resolveCallName(name: string, scope?: ScopeKey): string`：模型直呼的调用**实际会以哪个名字执行**——当该作用域处于 `code` 模式、且模型写下的是传输名的近似变形时返回保留的传输名，否则原样返回。agent 循环在追加 assistant 消息之前调用它，因为只修复分发只是修了一半：模型杜撰的名字同样会写进历史，而历史在此后的每一轮都会重放给提供方，于是这个名字的影响会超出它所在的那一次调用，甚至让整段会话都发不出去。
...
在 `code`（而非 `both`）下，该传输同时也是模型唯一可用的入口：模型直呼其他任何可见工具名，都会在创建执行时、早于 `tools/pre-execute`、审批 `ask` 和 guards 解析为 `UNKNOWN_TOOL`，因此没有任何一方会观察或批准一个注定失败的调用。拒绝信息会给出正确路径（`only \`run_code\` is callable directly — call \`<name>\` from inside a \`run_code\` program instead`），因为同一份提示词刚刚声明过那个工具，只说 `unknown tool` 会被读成部署损坏。对于任何地方都没有注册过的名字，分两种处理：若它是传输名本身的变形近似（`run_code_ide`、`runCode`，以及此前那版会回显坏名字的拒绝信息所诱发的 `run_code_ide_ide` 突变），该调用会被**回收**——改写为 `run_code` 并用调用本就携带的程序执行——因为拒绝信息只能劝告，而两轮打磨措辞之后，弱模型仍在重复并变异那个名字；其余未注册的名字仍然是拒绝，只指向 `run_code`，不把坏名字写成调用目标。若该名字仍保留在作用域的 `knownNames` 中（即被限制屏蔽掉的真实工具），则不回收，避免限制变成重定向。SDK 子分发携带外层执行的 `parent` token，不受此限制，因此程序保留 SDK 声明的全部绑定。参见[执行器塌缩 note](../../../.agents/notes/implemented/bug-fix/2026-08-07-code-mode-executor-collapse.md)、[Code Mode 基础](../../../.agents/notes/implemented/feature/2026-06-15-code-mode.md)、[类型化返回约定](../../../.agents/notes/implemented/feature/2026-07-20-code-mode-typed-tool-returns.md)和[代码运行时 seam](../../code-runtime/README.md)。可以运行 `pnpm run demo:code-mode` 试用。
