...
- `ctx.tools.resolveCallName(name: string, scope?: ScopeKey): string` The name a model-direct call will EXECUTE under — the reserved transport when the scope is in `code` mode and the model wrote a near-miss of it, else the name unchanged. The agent loop calls this before it appends the assistant message, because recovering the dispatch is only half the repair: the name the model invented is also written into history, which is replayed to the provider every later turn, so an invented name outlives its call and can make the whole conversation unsendable.
