...
锚点记录的是工具调用**实际执行时**所用的名字，未必是模型写下的那个：每个 `tool-call` 块在追加前都会经过 `ctx.tools.resolveCallName()`，注册表能够回收的名字（Code Mode 下 `run_code` 传输名的近似变形）会就地改写。这是历史记录层面的事，不是分发层面的事——assistant 消息在此后的每一轮都会被重放给提供方，因此一个提供方拒绝的名字不是让某一次调用失败，而是让**后续每一次请求都发不出去**，并且重试也救不回来，因为被拒绝的内容就躺在被重放的历史里。发生改写时，循环会在工具结果**之后**向下一步的收件箱投递一条 `tool-name-recovery` 提示，说明模型写了什么、实际跑的是什么；若改为静默回收，反而会强化那个杜撰的名字——真实观察到的一次会话里，`run_code_ide` 就是这样一路长成 `run_code_ide_ide` 乃至更长的。
