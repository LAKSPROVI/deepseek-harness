...
The anchor records the name a tool call will EXECUTE under, not necessarily the one the model wrote: each `tool-call` block passes through `ctx.tools.resolveCallName()` before the append, and a name the registry recovers (a Code Mode near-miss of the `run_code` transport) is rewritten in place. This is a history concern, not a dispatch one — the assistant message is replayed to the provider on every later turn, so a name the provider rejects makes every subsequent request unsendable rather than failing one call, and no retry can clear it because the rejection lives in the history being replayed. When a name is rewritten, the loop stages one `tool-name-recovery` notice into the next-step inbox AFTER the tool results, naming what the model wrote and what ran; recovering silently instead would reinforce the invented name, which is how one observed session grew `run_code_ide` into `run_code_ide_ide` and beyond.
