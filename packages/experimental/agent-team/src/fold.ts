...
const teamDebatePhaseSchema = z.enum(['positions', 'critique', 'rebuttal', 'verification', 'synthesis'])
const teamDebateStatusSchema = z.enum(['active', 'paused', 'completed'])
const teamDebateTransitionSchema = z.object({
 ...
 */
const TEAM_EVENT_VERSIONS: Record<TeamEventType, number> = {
  'team/member': 2,
  'team/task': 2,
  'team/debate': 1,
...
export function applyTeamEvent(state: TeamFoldState, event: SessionEvent): void {
  ...
  if (selector.version !== TEAM_EVENT_VERSIONS[event.type]) {
  ...
  switch (decoded.type) {
    case 'team/member': {
      ...
      if (prior === undefined) {
        ...
        if (prior.name !== member.name || prior.provider !== member.provider
          || prior.llmProvider !== member.llmProvider || prior.model !== member.model
          || prior.persona !== member.persona || prior.context !== member.context) {
    ...
    case 'team/debate': {
      ...
      if (prior === undefined) {
        ...
        if (prior.status !== 'completed' && prior.id !== debate.id) {
