...
精确模型 id `kr/gpt-5.6-sol-thinking-agentic` 在已配置路由上使用已验证的覆盖：选择器只提供 `off`、`low`、`medium`、`high`、`xhigh` 与 `max`；分派把 `off` 映射为 `none`，其余档位映射为自身。`minimal` 对其他模型仍然有效，但该 id 会在网络 I/O 前拒绝它。这是显式的模型验证，并非从端点 discovery 推断能力；已审计的 catalog 快照报告空的档位列表。
...
- **模态声明不经验证**：没有任何环节会去询问端点接受什么，因此声明了网关并不提供的 `image` 的模型会在 prompt 准入后被提供方拒绝。持久图片会留在历史中，同一个错误声明的模型可能再次失败。系统仍允许切换到纯文本模型，因为共享 LLM 运行时与适配器直接入口都会在该次请求中把不受支持的附件引用投影为稳定文本。
