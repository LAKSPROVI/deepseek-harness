...
The exact model id `kr/gpt-5.6-sol-thinking-agentic` has a verified override on configured routes: selectors offer exactly `off`, `low`, `medium`, `high`, `xhigh`, and `max`; with no explicit selection or route default, `off` is the model default and dispatch maps it to `none`, while the other levels map to themselves. An explicit selection or route default still wins. `minimal` remains valid for other models but is refused for this id before network I/O. This is an explicit model verification, not a capability inferred from endpoint discovery; the audited catalog snapshot reports an empty effort list.
...
Most listings disclose an id and nothing else; `context_window`/`context_length` and `max_output_tokens`/`max_tokens` are read when a gateway supplies them, entries without a usable id are skipped rather than failing the whole listing, and everything else the adopting surface still owes. A 9Router listing may additionally disclose `x_9r.capabilities_v1`: only `input.image.probe.state: proved` and `input.file.probe.state: proved` promote those modalities, in canonical `text`, `image`, `file` order. Claims, catalog flags, every other probe state, malformed data, and `agent_surface_v1` remain unknown and do not create an `inputModalities` field. The reply is read under a four-megabyte ceiling enforced on the bytes actually received — the endpoint is a URL the user typed, so a declared length is checked first but never trusted as the bound. An unreachable endpoint, a refused credential, a non-JSON body, and a body with no `data` array all fail with `DISCOVERY_FAILED` and a message naming the endpoint and, for a 401 or 403 alone, the credential. Cancellation during the body read surfaces as `ABORTED`, like a cancellation before the request went out.
...
- An outgoing tool name is clamped to `[A-Za-z0-9_-]`, at most 64 characters — Anthropic's documented maximum, and no gateway in front of it is more permissive. The name in an assistant message is whatever the MODEL emitted, and history is resent in full on every request, so an invented name does not fail one call: it makes every later request in that conversation unsendable, with no retry able to clear it because the rejection lives in the replayed history. This is a last-ditch guard — the agent loop already records the name that actually executed (see [agent-loop](../../core/agent-loop/README.md#loop-lifecycle-agentts)); the clamp exists so a name it could not recover degrades one call instead of killing the session. Tool results resolve their name through the same clamped assistant conversion, so call and result never disagree.
...
- pi-ai exposes a separate reasoning count when the provider reports one; the adapter maps it whenever defined, including zero, without changing output usage.
...
The selected catalog model receives `GenerateOptions.system`, history, tools, and sampling fields supported by pi-ai's common streaming API. Before context conversion, both the shared LLM route and the adapter's public `stream()` project image or generic-file blocks absent from that exact model's declared input modalities into deterministic metadata text; durable history remains unchanged. Each retained image is preceded by stable text naming its complete attachment id and actual request dimensions. When accumulated base64 image payload exceeds the route's `maxRequestImageBytes`, each offloaded image (oldest first) is replaced by fixed text that tells the model to read the file again when a path is available or ask the user to attach it again. Offloaded normalized attachments are not read or transformed. Provider-native replay metadata is restored only when the adapter validates it for the historical content.
...
Generated content affects later inputs only after the loop records it. A separately reported reasoning count appears as `reasoningTokens` alongside the provider's unchanged output count; providers that omit it leave that field absent.
