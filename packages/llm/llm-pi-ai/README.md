...
The exact model id `kr/gpt-5.6-sol-thinking-agentic` has a verified override on configured routes: selectors offer exactly `off`, `low`, `medium`, `high`, `xhigh`, and `max`; dispatch maps `off` to `none` and the other levels to themselves. `minimal` remains valid for other models but is refused for this id before network I/O. This is an explicit model verification, not a capability inferred from endpoint discovery; the audited catalog snapshot reports an empty effort list.
