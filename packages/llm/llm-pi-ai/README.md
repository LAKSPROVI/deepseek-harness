...
The exact model id `kr/gpt-5.6-sol-thinking-agentic` has a verified override on configured routes: selectors offer exactly `off`, `low`, `medium`, `high`, `xhigh`, and `max`; with no explicit selection or route default, `off` is the model default and dispatch maps it to `none`, while the other levels map to themselves. An explicit selection or route default still wins. `minimal` remains valid for other models but is refused for this id before network I/O. This is an explicit model verification, not a capability inferred from endpoint discovery; the audited catalog snapshot reports an empty effort list. Cross-package changes and live verification follow the [model capability runbook](../../../docs/cookbook/model-capability-agent-runbook.md).
