...
describe('draft-provider model discovery', () => {
  ...
  it('promotes only proved 9Router input probes in canonical order', async () => {
    ...
    expect(models.slice(0, 3)).toEqual([
      { id: 'image', name: 'image', inputModalities: ['text', 'image'] },
      { id: 'file', name: 'file', inputModalities: ['text', 'file'] },
      { id: 'both', name: 'both', inputModalities: ['text', 'image', 'file'] },
    ])
    expect(models.slice(3)).toEqual([
      ...inconclusive.map(({ id }) => ({ id, name: id })),
      { id: 'claim-false', name: 'claim-false' },
      { id: 'claim-true', name: 'claim-true' },
      { id: 'catalog-flag', name: 'catalog-flag' },
      { id: 'absent', name: 'absent' },
      { id: 'malformed-namespace', name: 'malformed-namespace' },
      { id: 'malformed-probe', name: 'malformed-probe' },
      { id: 'agent-only', name: 'agent-only' },
      { id: 'wrong-9r-child', name: 'wrong-9r-child' },
    ])
  })
