...
A structured debate is one durable snapshot with compare-and-set revision, participant names, round, phase, status, initial multimodal evidence, contribution speech records, and transition history. Its phases advance through `positions`, `critique`, `rebuttal`, `verification`, and `synthesis`. Debate contributions contain text, image, and file content blocks. Every participant in a phase must contribute during the current round before `advance` or `complete` can be committed. Only the Lead can start or update a debate; updates require its id and expected revision and apply `pause`, `resume`, `advance`, or `complete`. Status is `active`, `paused`, or `completed`. Pausing blocks protocol advancement but does not interrupt turns already running.

The `agentTeam` Session projection exposes durable member snapshots, non-deleted task snapshots, and the current debate (including structured evidence and speech contributions) for browser consumers. It deliberately excludes queued mailbox content. The `ui-subagent` Web view combines that projection with live roster status and presents the **Equipe de agentes** tab in Brazilian Portuguese: **Integrantes**, **Tarefas**, **Debate**, **Criar integrante** (with natural name normalization and saved reusable templates), **Orientar integrante**, **Interromper tarefa**, debate evidence, chronological speech transcript with author badges and downloadable files, phase readiness indicators, and Lead-authorized debate transitions. Presentation labels do not alter persisted status/phase/action values, generated Remote payloads, user-authored text, or provider diagnostics.
