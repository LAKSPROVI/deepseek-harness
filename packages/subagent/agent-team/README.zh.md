...
每个数字限制都必须是正的安全整数。`maxMembers` 默认且最高为 9 个 teammate，因此 Lead 加 teammate 不超过 10 个 agent；它统计所有曾 provision 的名字，包括失败成员，因为名字永不复用。`freshProvider` 与 `forkProvider` 分别选择 fresh 和 fork context 使用的 continuable-subagent transport。`maxTasks` 统计未删除任务，`maxDebateRounds` 默认为 8。mailbox 限额按 target 计算；字节限制覆盖完整投递帧，包括稳定 id 与发送者名称。`disposalTimeoutMs` 限制已获准创建、mailbox dispatch 与 Team 自有 Activation 的 settlement 时长，使插件 reload 与进程 shutdown 在异常时明确失败，而不是无限等待。
...
fresh child 不带 parent 历史 seed。fork child 只捕获一次 Lead 的已完成 turn 前缀，不包含正在执行 delegation 的 turn。每个 teammate snapshot 都持久保存可选的 `llmProvider`、`model` 与 child-only `persona`；continuation owner 在 cold resume 时恢复相同的选择与 persona。继承的 Team 记录带有旧 Root 的 `TeamId`，普通 fork 成为独立运行时 Root 后会忽略这些记录。roster 之外、由 provider 管理的 subagent 不会被误认为嵌套 Team Lead。
...
每条已投递 peer 消息都是用户角色消息。第一个短文本块包含稳定消息 id 与发送者，之后原样附加发送者的内容块。roster、task、debate 和 mailbox 记录本身只存在于日志，不进入派生模型历史。

#### Token 影响

每次 peer 投递都会把发送者前缀与消息内容加入 target 历史。roster、task 与 debate 变更不增加模型 token；其面向模型的呈现属于 `@deepseek-ai/dsh-tool-agent-team` 结果。
