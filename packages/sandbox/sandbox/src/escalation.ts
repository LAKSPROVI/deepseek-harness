 ...
 * strictly-wider ladder, idempotent current-mode normalization, argument-pairing
 * validation, model-facing denial/hint markers, and {@link approveEscalation}.
 * Wider requests resolve through user approval BEFORE anything executes. One home keeps the two
...
export interface EscalationRequest {
  ...
  /** The call's effective mode (session override ?? composition default) the request may retain or strictly widen. */
...
/**
 * Resolve a sandbox-escalation request BEFORE anything executes. A valid
 * target equal to the call's effective mode is an idempotent no-op that never
 * prompts; a wider target resolves through approval, while a narrower or
 * otherwise invalid target fails closed. Returns the mode to stamp onto
 * exactly this call; throws distinct verbatim text for every non-grant path —
...
export async function approveEscalation<A, C>(request: EscalationRequest, approval: EscalationApproval<A, C>): Promise<SandboxMode> {
  ...
  if (mode === effectiveMode && ESCALATION_TARGETS.includes(mode as SandboxMode)) return effectiveMode
