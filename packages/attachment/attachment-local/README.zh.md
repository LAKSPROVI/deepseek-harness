...
通用文件共用同一套对象目录、原子发布路径和完整性校验，并按字节原样存储，因为后端绝不会对它们解码、解压提取、转码或执行。`saveFileStream` 消费有界 chunk，在发布前校验可选的预期长度，递增计算摘要，fsync 私有暂存文件，再把经过校验的对象硬链接进内容寻址存储；取消、超限和长度不符都会清除暂存状态。相同字节会去重，而每个 receipt 都使用进程本地 HMAC 密钥验证自身的规范化元数据与确切会话 scope。`saveFiles` 保留为有界兼容路径。引用只记录 sha256 标识符、字节长度、规范化媒体类型和可选展示名称：提交名称会同时按 `/` 和 `\` 取末段，去除控制字符和首尾空白，截断到 255 个字符，并在结果为空、`.` 或 `..` 时整体丢弃。`readFileStream` 递增校验精确长度和摘要。文件不经过只约束图片变换的压缩限流器。

`DSH_HOME` 按共享路径策略解析：显式配置、`$DSH_HOME`，最后是 `~/.dsh`。会话日志只包含引用和经过校验的元数据，绝不包含这个宿主路径或上传 receipt。流式与内存读取都会保留取消语义，不会把它转换为存储失败。
