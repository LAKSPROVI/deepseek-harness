...
export class CommandUiRuntime extends Service implements CommandUiContract {
   ...
   * through a command declaring attachment acceptance. Every other command
   ...
   * attachments stay in place; nothing executes and nothing is dropped.
   */
   ...
   * composer keeps the submission (draft and attachments) for correction.
  ...
  private async execute(
    ...
    // A submission carrying attachments consumes them only on handler success;
    // an error outcome keeps the complete envelope in the composer.
