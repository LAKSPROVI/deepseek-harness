...
async function bench(opts: BenchOptions = {}) {
  ...
  const executeCalls: Array<{ sessionId: SessionId; line: string; attachments: readonly SubmitAttachment[] }> = []
  ...
  const commandsRemote = {
    ...
    execute: async (sessionId: SessionId, line: string, attachments: readonly SubmitAttachment[] = []) => {
      executeCalls.push({ sessionId, line, attachments })
      return await carried(async () => {
...
describe('decorations (bare-invocation UI on host commands)', () => {
  ...
  it('an unavailable decoration falls through to the host bare path (detached execute)', async () => {
    ...
    expect(executeCalls).toEqual([{ sessionId: sid('s1'), line: '/plan', attachments: [] }])
  })
...
describe('matchEnter envelope policy (attachments)', () => {
  const signal = () => new AbortController().signal
  ...
  it('a leadingInput command not declaring acceptance refuses; a declaring one claims with attachments minted', async () => {
  ...
  it('bare popup routes refuse attachments: contribution and decorated host both stay closed', async () => {
    const { command, source, mint, warm } = await bench()
    command.register(themeContribution())
  ...
  it('bare host detached execute refuses attachments before any RPC', async () => {
    const { source, warm, executeCalls } = await bench()
    await warm(proj('s1'))
  ...
  it('claim.submit forwards the attachments to execute; consumption follows the handler outcome', async () => {
    ...
    if (outcome === undefined || outcome === 'handled' || !('claim' in outcome)) throw new Error('expected claim')
    // Handler error: the error outcome keeps draft and attachments in the composer.
    await expect(outcome.claim.submit('x', new Context(), [png]))
      .resolves.toEqual({ kind: 'error', text: 'handler refused' })
    expect(executeCalls).toEqual([{ sessionId: sid('s1'), line: '/vision x', attachments: [png] }])
...
describe('execute payload', () => {
  it('claim.submit addresses the session; admitted outcomes stay off the composer (flow card owns them)', async () => {
    ...
    expect(executeCalls).toEqual([{ sessionId: sid('s1'), line: '/goal ship it', attachments: [] }])
