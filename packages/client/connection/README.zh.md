...
- **JSON `/api` 桥把每个请求体整体缓冲在内存里**：`maxRequestBodyBytes`（默认 600 MiB）仍是 RPC 请求体与 base64 图片／兼容上传的驻留内存上界。served Web 的通用文件经 `/api/session.file` 绕过它；该 POST 与 GET 使用带背压的有界 chunk 复制。fixture 与自定义 `__DSH_TRANSPORT__` carrier 不暴露原始传输，继续使用有界 base64 兼容路径。
