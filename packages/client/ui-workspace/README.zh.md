...
Session 行内的 fork 操作在源会话最后一个已完成轮次处 fork，在客户端递增继承的持久化标题后再打开子会话；尾部半角或全角括号编号会原样式递增，无编号标题追加 ` (1)`。源会话与子会话在 workspace 组内始终作为同级行展示，谱系只保留为 session 数据。Fork 或改名失败都不会改变当前选中项，改名失败时已创建的子会话仍会留在列表中。行菜单还提供状态切换操作：**标为未读／标为已读**与**标为已完成／标为未完成**。

本包还贡献了会话页头实用工具与操作扩展：
- **会话备注与待办提醒（`SessionNotesPopover`）**：注册至 `conversation.session.header.utilities`。每个聊天会话均包含独立的备注面板，支持自由记事（按 `sessionId` 自动持久化至本地存储）、带到期时间／日期的未来提醒事项、复选框完成标记、剪贴板一键复制以及直接插入当前聊天输入框。
- **快速上下文与新会话操作（`SessionContextActions`）**：注册至 `conversation.session.header.actions`。允许在同一屏幕同工作区内直接开启全新空白会话、基于上一会话结构化摘要开启新会话以清洗上下文节省 Token，或快速发起分支 Fork。

Session 行渲染运行时的实时 `pendingInteraction` 分类：审批显示**等待审批**，计划审阅显示**计划待审**，普通问题显示**等待回答**。每个待处理交互都使用一枚琥珀色警告点，优先级高于运行指示器；普通行的悬浮卡片重复显示本地化状态，普通行和搜索结果行则都以相同文本提供面向辅助技术的视觉隐藏标签。运行状态使用带微呼吸脉冲动效的蓝色点阵；未读会话显示带发光光晕的明亮蓝点；稍后处理会话显示带暖橙色光晕的点（**稍后处理** / Concluir Depois）；已完成会话显示实心绿色指示点；已结束会话显示带紫色光晕的点（**已结束** / Finalizado）；空闲行会保留空的状态槽位。

在 Workspace 分组树上方，浏览器提供专用的**进行中与近期会话**（Recent & In Progress）区域，汇聚当前活跃会话（运行中、包含运行中子智能体、等待用户交互）、未读会话、稍后处理会话以及刚执行结束的任务。活跃任务完成后会自动标为已完成（`completed`）并保留在顶部区域，直至用户手动进行状态分流或更改。每条近期会话均带有紧凑的所属 Workspace 徽标（或未分组标识），并支持一键直达。点击或打开会话不会自动将其从近期会话中移除。将会话分流为已完成或已结束会将其移出顶部近期区域，并在下方分组行中保留对应的状态指示；而设为**稍后处理**则会以高优先级（`score: 28`）将其持续钉在顶部近期区域，方便后续跟进。
