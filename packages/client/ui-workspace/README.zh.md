...
Session 行渲染运行时的实时 `pendingInteraction` 分类：审批显示**等待审批**，计划审阅显示**计划待审**，普通问题显示**等待回答**。每个待处理交互都使用一枚琥珀色警告点，优先级高于运行指示器；普通行的悬浮卡片重复显示本地化状态，普通行和搜索结果行则都以相同文本提供面向辅助技术的视觉隐藏标签。运行状态使用动态蓝色旋转点阵；未读会话显示带发光光晕的明亮蓝点；已完成会话显示实心绿色指示点；空闲行会保留空的状态槽位。

在 Workspace 分组树上方，浏览器提供专用的**进行中与近期会话**（Recent & In Progress）区域，汇聚当前活跃会话（运行中、包含运行中子智能体、等待用户交互）与未读会话。每条近期会话均带有紧凑的所属 Workspace 徽标（或未分组标识），并支持一键直达。点击或打开会话不会自动将其从近期会话中移除；会话会持续保留在近期列表中，直至用户显式对其设置指示器（如标为已完成或标为已读）。将会话标为已完成会将其移出顶部近期区域，并在下方分组行中保留绿色已完成状态指示。
