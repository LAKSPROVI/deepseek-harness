...
The `conversation.view` contribution appears as **Equipe de agentes** and consumes the `agentTeam` Session projection, the Lead Session's read-only `session.models` directory, and generated `agentTeams` Remote actions. **Criar integrante** presents linked provider/model selectors: the inherited option omits both route fields, while an explicit provider requires one model advertised by that provider. A catalog failure preserves inherited creation and exposes retry; provider-local failures leave successful groups selectable. The remaining Brazilian Portuguese interface covers **Integrantes**, **Tarefas**, **Debate**, guidance, current-turn interruption, protocol transitions, loading states, local failures, help, and accessibility labels. Stable protocol values remain English on the wire and in persistence; the component translates only presentation and leaves names, topics, notes, descriptions, personas, and provider diagnostics unchanged. The [user guide](../../../docs/user/guide/agent-teams.md) owns the click path, and the [Agent Teams subsystem](../../../docs/subsystems/agent-team.md) owns the projection and composition map.
